...
import type { Attachment, FileAnalysisMode, FileAnalysisResult, RFQAnalysis, SupplierAnalysis } from '@/lib/types'
...
function RiskGauge({ score }: { score: number }) {
  ...
  const color =
    clamped <= 35 ? 'bg-emerald-500'
    : clamped <= 65 ? 'bg-amber-500'
    : 'bg-red-500'
  const textColor =
    clamped <= 35 ? 'text-emerald-300'
    : clamped <= 65 ? 'text-amber-300'
    : 'text-red-300'
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <span className="text-[11px] text-gray-500 font-medium uppercase tracking-wide">Risk Score</span>
        <span className={`text-sm font-bold tabular-nums ${textColor}`}>{clamped}<span className="text-[11px] font-normal text-gray-600">/100</span></span>
      </div>
...
function Section({ icon: Icon, title, children, accent = 'blue' }: {
  ...
  return (
    <div className="space-y-2">
      <div className={`flex items-center gap-2 text-xs font-semibold uppercase tracking-wide px-2 py-1.5 rounded-lg border ${colors}`}>
        <Icon className="h-3.5 w-3.5" />
        {title}
      </div>
      {children}
...
function BulletList({ items, variant = 'neutral' }: { items: string[]; variant?: 'neutral' | 'warning' | 'positive' }) {
  ...
  return (
    <div className="rounded-xl border border-white/[0.07] bg-[#0d1220] overflow-hidden">
      ...
      >
        ...
        {state.status === 'analyzing' || state.status === 'signing' ? (
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-white/[0.05]">
          {(state.status === 'signing' || state.status === 'analyzing') && (
          ...
          {state.status === 'error' && (
            <div className="flex items-start gap-2.5 pt-3 rounded-lg bg-red-500/[0.06] border border-red-500/20 px-3 py-2.5">
          ...
          {state.status === 'done' && (
            <div className="pt-4 space-y-4">
              {mode === 'rfq' && state.result.rfq && (
                <RFQResultCard result={state.result.rfq} />
              )}
              {mode === 'supplier' && state.result.supplier && (
                <SupplierResultCard result={state.result.supplier} />
              )}
...
export function FileAnalysisPanel({ mode, onResults, disabled }: Props) {
  // Map from Supabase path → current analysis state for that file
  ...
  const analyzeFile = useCallback(async (attachment: Attachment) => {
    ...
    try {
      // Generate a short-lived signed URL the API can fetch
      const supabase = getSupabase()
      const { data: signed, error: signErr } = await supabase.storage
        .from('attachments')
        .createSignedUrl(path, 300)   // 5-minute URL — enough for the API to fetch it
  ...
  const handleAttachmentsChange = useCallback((attachments: Attachment[]) => {
    ...
    // If a file was removed, clean up its analysis state
  ...
  const anyAnalyzing = [...states.values()].some(
    (s) => s.status === 'signing' || s.status === 'analyzing',
  )
  const doneCount = [...states.values()].filter((s) => s.status === 'done').length

  // Map path → file name for the result cards
  // We need to keep a name lookup separate from the Attachment list
  const fileNames = useRef<Map<string, string>>(new Map())

  const handleAttachmentsChangeWithNames = useCallback((attachments: Attachment[]) => {
    for (const att of attachments) {
      fileNames.current.set(att.path, att.name)
    }
  ...
  return (
    <div className="space-y-4">
      {/* Upload zone */}
      <AttachmentUploader
        onChange={handleAttachmentsChangeWithNames}
        disabled={disabled}
      />
      ...
      {states.size > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/[0.02] border border-white/[0.05]">
          ...
          <div className="ml-auto flex items-center gap-1 text-[11px] text-gray-600">
            ...
            <span>
              {mode === 'rfq' ? 'RFQ document mode' : 'Supplier quotation mode'}
            </span>
          </div>
        </div>
      )}
      ...
      {[...states.entries()].map(([path, state]) => (
        <AnalysisResultCard
          key={path}
          state={state}
          fileName={fileNames.current.get(path) ?? path.split('/').pop() ?? 'file'}
          mode={mode}
        />
      ))}
    </div>
