...
import { Crown, Lock, Sparkles, X, Check, Zap, BarChart3, GitCompare, Brain } from 'lucide-react'
        ...
        <div className="flex flex-col items-center gap-3 text-center px-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1 leading-relaxed max-w-[200px] mx-auto">
              Upgrade to unlock AI insights, advanced analytics, and more.
            </p>
          </div>
          <button
...
// ── ProGate ───────────────────────────────────────────────────────────────────
// Client component. Parent (server component) fetches subscription and passes isPro.
// When not pro: blurs children and shows upgrade overlay.
// When pro: renders children transparently.

interface Props {
  isPro: boolean
  children: React.ReactNode
  feature?: string
  placeholder?: React.ReactNode  // static mock shown blurred; falls back to children
  className?: string
}

export function ProGate({ isPro, children, feature, placeholder, className }: Props) {
...
// ── UpgradeCard — compact inline upgrade prompt ────────────────────────────────
...
export function UpgradeCard({
  ...
  description = 'Get AI insights, advanced analytics, comparisons, and more.',
  compact = false,
}: UpgradeCardProps) {
  const [modalOpen, setModalOpen] = useState(false)

  const FEATURES = [
    { icon: Brain,      label: 'AI Insights & recommendations' },
    { icon: BarChart3,  label: 'Advanced analytics & charts' },
    { icon: GitCompare, label: 'Supplier comparison engine' },
      ...
      <div className={`rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-950/60 to-blue-950/40 ${compact ? 'p-4' : 'p-6'} relative overflow-hidden`}>
        ...
        <div className="relative flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <button
              onClick={() => setModalOpen(true)}
              className={`mt-3 flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-blue-600 hover:from-violet-500 hover:to-blue-500 text-white font-semibold shadow-lg shadow-violet-600/25 transition-all active:scale-95 ${compact ? 'px-3 py-1.5 text-xs' : 'px-4 py-2 text-sm'}`}
            >
              <Crown className="h-3.5 w-3.5" />
              Upgrade to Pro
            </button>
...
// ── LockIcon — small inline "Pro" badge ───────────────────────────────────────
